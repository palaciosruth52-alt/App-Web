...
if choice == "Proyectos":
    ...
    for p in c.fetchall():
        ...
        # Lógica de Editar
        if col3.button("✏️", key=f"edit_p_{p[0]}"):
...
# --- ENTIDAD 2: TAREAS ---
    ...
    if proyectos_disponibles:
        ...
        # READ, UPDATE & DELETE
        c.execute("SELECT t.id, p.nombre, t.titulo, t.estado, t.id FROM tareas t JOIN proyectos p ON t.proyecto_id = p.id")
        for t in c.fetchall():
            ...
            if col4.button("✏️", key=f"edit_t_{t[0]}"):
                st.session_state[f"edit_t_{t[0]}"] = True
            
            if st.session_state.get(f"edit_t_{t[0]}", False):
                with st.form(f"form_edit_t_{t[0]}"):
                    ...
                    if st.form_submit_button("Actualizar"):
                        ...
                        conn.commit()
                        st.session_state[f"edit_t_{t[0]}"] = False
                        st.rerun()
...
# --- VISTA GENERAL ---
    ...
    st.metric("Proyectos", c.fetchone()[0])
